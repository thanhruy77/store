document.addEventListener("DOMContentLoaded", function () {
    var danhsachdonhang = JSON.parse(localStorage.getItem('dsdonhangxacnhan'));
    ...
    if (danhsachdonhang && danhsachdonhang.length > 0) {
        danhsachdonhang.forEach(function (donhang) {
            ...
            // nút xác nhận
            var xacnhanButton = document.createElement('button');
            xacnhanButton.classList.add('chitiet-button');
            xacnhanButton.textContent = 'Giao hàng';
            donhangElement.appendChild(xacnhanButton);


            // Sự kiện click cho nút giao hàng
            xacnhanButton.addEventListener('click', function () {
                var currentDonHang = donhang; // Lưu đơn hàng hiện tại vào biến currentDonHang
                if (confirm("Bạn có chắc chắn muốn giao đơn hàng này không?")) {
                    // Xác nhận đơn hàng và cập nhật localStorage
                    var index = danhsachdonhang.indexOf(currentDonHang);
                    if (danhsachdonhang[index].xacnhan !== "Chờ xác nhận" && danhsachdonhang[index].xacnhan !== "Đang giao" ) {
                        danhsachdonhang[index].xacnhan = "Đang giao";
                        // Lấy thông tin của đơn hàng
                        var donhangxacnhan = {
                            hoten: currentDonHang.hoten,
                            sdt: currentDonHang.sdt,
                            diachi: currentDonHang.diachi,
                            xacnhan: currentDonHang.xacnhan,
                            id: currentDonHang.id,
                            sanpham: currentDonHang.sanpham
                        };

                        // Thêm đơn hàng đã xác nhận vào danh sách 'dsdanggiao'
                        var dsgiaohangxacnhan = localStorage.getItem('dsdanggiao') ? JSON.parse(localStorage.getItem('dsdanggiao')) : [];
                        dsgiaohangxacnhan.push(donhangxacnhan);
                        localStorage.setItem('dsdanggiao', JSON.stringify(dsgiaohangxacnhan));
                        danhsachdonhang.splice(index,1);
                        // Cập nhật danh sách đơn hàng và cập nhật localStorage
                        localStorage.setItem('dsdonhangxacnhan', JSON.stringify(danhsachdonhang));

                        location.reload();
                    } else {
                        alert("Đơn hàng này đã được xác nhận hoặc đã đang giao trước đó!");
                    }
                }
            });
