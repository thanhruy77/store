document.addEventListener("DOMContentLoaded", function () {
    var danhsachdonhang = JSON.parse(localStorage.getItem('dsdanggiao'));
    ...
    if (danhsachdonhang && danhsachdonhang.length > 0) {
        danhsachdonhang.forEach(function (donhang) {
            ...
            xacnhanButton.textContent = 'Đã giao hàng';
            ...
            xacnhanButton.addEventListener('click', function () {
                ...
                if (confirm("Bạn có chắc chắn đơn hàng này đã giao không?")) {
                    // Xác nhận đơn hàng và cập nhật localStorage
                    var index = danhsachdonhang.indexOf(currentDonHang);
                    if (danhsachdonhang[index].xacnhan == "Đang giao" ) {
                        danhsachdonhang[index].xacnhan = "Đã giao hàng";
                        ...
                        // Thêm đơn hàng đã xác nhận vào danh sách 'dsdonhangxacnhan'
                        var dsgiaohangxacnhan = localStorage.getItem('dsdonhangdagiao') ? JSON.parse(localStorage.getItem('dsdonhangdagiao')) : [];
                        dsgiaohangxacnhan.push(donhangxacnhan);
                        localStorage.setItem('dsdonhangdagiao', JSON.stringify(dsgiaohangxacnhan));
                        danhsachdonhang.splice(index,1);
                        // Cập nhật danh sách đơn hàng và cập nhật localStorage
                        localStorage.setItem('dsdanggiao', JSON.stringify(danhsachdonhang));

                        location.reload();
                    } else {
                        alert("Đơn hàng này chưa giao!");
                    }
                }
            });
